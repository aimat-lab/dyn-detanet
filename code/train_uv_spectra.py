...
with open(csv_path, newline='', encoding='utf-8') as csvfile:
    ...
    for row in csv_reader:
        ...
        spectrum_funcs = spectras[idx] 
        fun = spectrum_funcs['fun']
        spectrum_value = sum(func(freq_val) for func in fun)

        freq_centers  = spectrum_funcs['freqs']
        freq_intensities = spectrum_funcs['amps']

        print("freq_center", len(freq_centers))
        print(freq_centers)
        
        freq_centers= torch.tensor(freq_centers, dtype=torch.float32).unsqueeze(0)
        #print("freq_centers.shape", freq_centers.shape)
        freq_intensities = torch.tensor(freq_intensities, dtype=torch.float32).unsqueeze(0)
        ...
        if balance:
            ...
            if spectrum_value > 0.018 and spectrum_value < 0.02:
                dataset.append(data_entry)
                count += 1    
...
print("dataset[0] :", ex1.idx, ex1.freq, )
print("dataset[5] :", ex2.idx, ex2.freq,)
...
wandb.init(
    ...
    project="Detanet-freq-learn-fails",
    name=f"freq-readout", 
...
model = DetaNet(num_features=128,
                    ...
                    atom_ref=None,
                    scale=1.0,
                    scalar_outsize=len_freq_centers, # ==241
                    irreps_out= None,
                    summation=True,
                    norm=False,
                    out_type='scalar', # '2_tensor',
                    grad_type=None,
                    device=device)
...
torch.save(model.state_dict(), current_dir + f'/trained_param/ee_polarizabilities_spec.pth')
...
plt.legend()
plot_path = os.path.join(current_dir, f'ee_polarizabilities_spec.png')
